        ...
        >
          {portfolio.map((item, index) => {
            ...
            <motion.div key={item.title} variants={itemVariants}>
                <Card className={`group relative h-full overflow-hidden border-border/50 bg-background/40 backdrop-blur-sm transition-all duration-300 hover:border-primary/40 hover:bg-background/60 ${isClickable ? 'cursor-pointer' : ''}`}>
                <CardContent className="relative p-0">
                  {isClickable ? (
                    <a
                      ...
                      aria-label={`View ${item.title}${isExternalLink ? ' (opens in new tab)' : ''}`}
                      className="relative z-0 flex h-full flex-col transition-transform duration-300 group-hover:scale-[1.02] cursor-pointer"
                    >
                      {/* Gradient overlay on hover */}
                      <motion.div
                        className="pointer-events-none absolute inset-0 z-10 bg-gradient-to-t from-background/90 via-background/50 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100"
                        initial={false}
                      />
                    ...
                    <div className="relative aspect-[4/3] w-full overflow-hidden bg-primary/20">
                      ...
                      {/* Floating badge - only show for external links */}
                      {isExternalLink && (
                        <motion.div
                          className="pointer-events-none absolute left-4 top-4 z-30 rounded-full border border-background/20 bg-background/80 px-3 py-1.5 text-[10px] font-medium text-foreground backdrop-blur-md shadow-lg"
                      ...
                      {/* View project button - appears on hover, only for clickable items */}
                      {isClickable && (
                        <div className="pointer-events-none absolute bottom-4 right-4 z-30 opacity-0 transition-all duration-300 group-hover:scale-100 group-hover:opacity-100">
                      ...
                      {item.tags && item.tags.length > 0 && (
                        <motion.div
                          className="pointer-events-none absolute bottom-4 left-4 z-30 rounded-full border border-background/20 bg-background/80 px-3 py-1.5 text-[11px] font-medium text-muted-foreground backdrop-blur-md shadow-lg"
